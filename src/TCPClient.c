...
/**
 * @brief Add newline terminale consted type.
 * 
 */
void newline_messagebox(){
    ...
    fflush(stdout); // Tamponta bulunan verileri bosalt
}

/**
 * @brief Get the terminal col object
 * 
 * @return int Terminal Size or Default value
 */
...
/**
 * @brief Get the terminal row object
 * 
 * @return int Terminal Size or Default value
 */
...
/**
 * @brief (Client) can send message from server *without encryption.
 * 
 * @param network_socket 
 * @param server_address 
 * @param username 
...
void secure_user_send_message(int network_socket, struct sockaddr *server_address, char *username){
  ...
  char *terminal_line = NULL;     // Get the user terminal typed line.
  size_t terminal_line_size = 0;  // Hold the user terminal typed line size.
  char stack_buffer[BUFFER_SIZE]; // Buffer for message and encrypted message
  unsigned int err_count = 0;     // Getline error count
  bool max_lenght_err = false;
  newline_messagebox();           // Call once.

  while (true) { // Start the infinite loop.
    if (terminal_line != NULL) { // I don't understand
      free(terminal_line);
    }
    ...
    // Call getline if getline have a error than return value is need to be -1
    ...
    if(return_char_count == (unsigned int)(-1)){
      ...
      err_count++;
      continue;
    } else if((size_t)256 < strlen(terminal_line)){
      int terminal_height = get_terminal_height()/2 + 1;
      max_lenght_err = true;
      
      printf("\033[%dB", terminal_height); // terminal_height satır aşağı
      fprintf(stdout, RESET BLKHB HWHT"Max character size is 256\\" RESET BLKHB HRED"%zu" RESET, return_char_count);
      printf("\033[%dA", terminal_height - 1); // terminal_height satır yukariya
      fflush(stdout);
      newline_messagebox();
      fflush(stdout);
      terminal_line = NULL;
    ...
    } else if((size_t)256 > strlen(terminal_line) && max_lenght_err){
      int terminal_height = get_terminal_height()/2 + 1;
      max_lenght_err = false;
        
        printf("\033[%dB", terminal_height - 1); // terminal_height satır aşağı
        printf("\033[K");                        // Satiri temizle
        fflush(stdout);                          // stdout bosalt
    ...
    // Combine username and terminal line in the current stack_buffer.
    snprintf(stack_buffer, sizeof(stack_buffer), "%s: %s", username, terminal_line);

// ENC START
    LOG_SUCCESS(client, "Getline encryption start");
    ...
    if (!return_encryption) {
        ...
        char *hex_ret = str_to_hex(return_encryption, BUFFER_SIZE);
        LOG_SUCCESS(client, "Encrypted message       : %s", (const char*)hex_ret);
        LOG_SUCCESS(client, "Encrypted message lenght: %zu", strlen(return_encryption));
    }

// ENC END
    if(send_secure(network_socket, return_encryption) != 0)
    {
        LOG_SUCCESS(client, "Encrypted message send server succesfully.");
        free(return_encryption);
    }
  }
}
